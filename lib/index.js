...
const exeProcessName = {
  darwin: 'wechatwebdevtools',
  win32: 'wechatdevtools.exe'
}
...
module.exports = (options) => {
  ...
  const processName = exeProcessName[platform] || ''
  ...
  switch (args[0]) {
    ...
    case '--quit':
    case '-q': {
      console.log(`退出微信开发者工具`)
      if (processName) {
        fkill(processName, {
          force: true
        }).then(() => {
          console.log(chalk`{green 已退出} 微信开发者工具`)
        }).catch(() => {
          console.log(`未找到启动的开发者工具`)
        })
      }
      return
    }
